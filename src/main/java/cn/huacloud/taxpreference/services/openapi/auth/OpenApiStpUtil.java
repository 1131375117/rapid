 ...
 * @author kong
 */
public class OpenApiStpUtil {

    /**
     * 账号类型标识
     */
    public static final String TYPE = "open-api";

    public static final String OPEN_USER_ID_NAME = "OpenUserId";

    /**
     * OpenAPI独立token配置
     */
    public static SaTokenConfig openApiTokenConfig;

    /**
     * 用来保存OpenUserId
     */
    public static final ThreadLocal<String> openUserIdThreadLocal = new ThreadLocal<>();
    ...
    public static StpLogic stpLogic = new StpLogic(TYPE) {
        ...
        public SaTokenConfig getConfig() {
            if (openApiTokenConfig != null) {
                return openApiTokenConfig;
            }
    ...
    /**
     * 在OpenAPI中获取OpenUserId，需要在接口上添加 @OpenApiCheckOpenUserId 注解才能包装可以获取到ID
     * @return
     */
    public static String getOpenUserId() {
        return openUserIdThreadLocal.get();
    }

    /**
     * 在OpenAPI中获取自动创建的渠道用户的用户ID，需要在接口上添加 @OpenApiCheckOpenUserId 注解才能包装可以获取到ID
     * @return
     */
    public static Long getConsumerUserId() {
        String openUserId = getOpenUserId();
        String key = getConsumerUserIdKey(openUserId);
    ...
    /**
     * 设置openUserId对应的consumerUserId
     * @param openUserId
     * @param consumerUserId
     */
    public static void setConsumerUserId(String openUserId, Long consumerUserId) {
        String key = getConsumerUserIdKey(openUserId);
        long timeout = stpLogic.getConfig().getTimeout();
        SaManager.getSaTokenDao().set(key, "" + consumerUserId, timeout);
    }
     ...
     * @return
     */
    private static String getConsumerUserIdKey(String openUserId) {
        return "open-api:open-user-id:" + openUserId;
    }
